...
class GenerarFeedbackService:
    ...
    async def ejecutar(self, feedback: PreguntasDto):
        ...
        total_respuestas = len(feedback.proceso_entrevista)
        ...
        for bloque in range(numero_bloques):
            ...
            respuestas_bloque = feedback.proceso_entrevista[inicio_bloque:fin_bloque]
            ...
            feedback_procesado = self.procesar_respuesta(feedback_response['answer'])

            # Elimina primer elemento en caso de que el modelo genere una separacion no deseada
            if feedback_procesado[0] == '':
                feedback_procesado.pop(0)
            feedback_total.extend(feedback_procesado)

        # Asignar el feedback procesado de vuelta al dto de feedback
        for i, feedback_item in enumerate(feedback.proceso_entrevista):
            if i < len(feedback_total):
                feedback_item.feedback = feedback_total[i]

        fin = time.time()  # Finalizar el contador de tiempo
        print(f"El proceso ha tardado {fin - inicio} segundos.")  # Imprimir la duración del proceso

        await self.kafka_producer_service.send_message(feedback.dict(), 'feedbackListenerTopic')
    ...
    def procesar_respuesta(self, respuesta):
        ...
        feedbacks = respuesta.split("**Feedback:**")
        return feedbacks
